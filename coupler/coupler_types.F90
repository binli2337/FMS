...
#include <fms_platform.h>
  ...
  use fms_io_mod,        only: restart_file_type_r8
...
#include<file_version.h>
  ...
  type, public :: coupler_3d_values_type_r8
    character(len=48)       :: name = ' '  !< The diagnostic name for this array
    real(DOUBLE_KIND), pointer, contiguous, dimension(:,:,:) :: values => NULL() !< The pointer to the
                                           !! array of values for this field; this
                                           !! should be changed to allocatable
    logical                 :: mean = .true. !< mean
    logical                 :: override = .false. !< override
    ...
    logical                 :: may_init = .true. !< If true, there is an internal method
                                           !! that can be used to initialize this field
                                           !! if it can not be read from a restart file
  end type coupler_3d_values_type_r8
  ...
  type, public :: coupler_3d_field_type_r8
    character(len=48)                 :: name = ' ' !< name
    integer                           :: num_fields = 0 !< num_fields
    type(coupler_3d_values_type_r8), pointer, dimension(:) :: field => NULL() !< field
    character(len=128)                :: flux_type = ' ' !< flux_type
    character(len=128)                :: implementation = ' ' !< implementation
    real(DOUBLE_KIND), pointer, dimension(:)       :: param => NULL() !< param
    logical, pointer, dimension(:)    :: flag => NULL() !< flag
    integer                           :: atm_tr_index = 0 !< atm_tr_index
    character(len=128)                :: ice_restart_file = ' ' !< ice_restart_file
    character(len=128)                :: ocean_restart_file = ' ' !< ocean_restart_file
    type(restart_file_type_r8), pointer  :: rest_type => NULL() !< A pointer to the restart_file_type
                                                             !! that is used for this field.
    logical                           :: use_atm_pressure !< use_atm_pressure
    logical                           :: use_10m_wind_speed !< use_10m_wind_speed
    logical                           :: pass_through_ice !< pass_through_ice
    real(DOUBLE_KIND)                 :: mol_wt = 0.0d0 !< mol_wt
  end type coupler_3d_field_type_r8
  ...
  type, public :: coupler_3d_bc_type_r8
    integer                                            :: num_bcs = 0  !< The number of boundary condition fields
    type(coupler_3d_field_type_r8), dimension(:), pointer :: bc => NULL() !< A pointer to the array of boundary condition fields
    logical    :: set = .false.       !< If true, this type has been initialized
    integer    :: isd, isc, iec, ied  !< The i-direction data and computational domain index ranges for this type
    integer    :: jsd, jsc, jec, jed  !< The j-direction data and computational domain index ranges for this type
    integer    :: ks, ke              !< The k-direction index ranges for this type
  end type coupler_3d_bc_type_r8
  ...
  type, public    :: coupler_2d_values_type_r8
    character(len=48)       :: name = ' '  !< The diagnostic name for this array
    real(DOUBLE_KIND), pointer, contiguous, dimension(:,:) :: values => NULL() !< The pointer to the
                                           !! array of values for this field; this
                                           !! should be changed to allocatable
    ...
    logical                 :: may_init = .true. !< If true, there is an internal method
                                           !! that can be used to initialize this field
                                           !! if it can not be read from a restart file
  end type coupler_2d_values_type_r8
  ...
  type, public    :: coupler_2d_field_type_r8
    character(len=48)                 :: name = ' ' !< name
    integer                           :: num_fields = 0 !< num_fields
    type(coupler_2d_values_type_r8), pointer, dimension(:)   :: field => NULL() !< field
    character(len=128)                :: flux_type = ' ' !< flux_type
    character(len=128)                :: implementation = ' ' !< implementation
    real(DOUBLE_KIND), pointer, dimension(:)       :: param => NULL() !< param
    logical, pointer, dimension(:)    :: flag => NULL() !< flag
  ...
  end type coupler_2d_field_type_r8
  ...
  type, public    :: coupler_2d_bc_type_r8
    integer                                            :: num_bcs = 0  !< The number of boundary condition fields
    type(coupler_2d_field_type_r8), dimension(:), pointer :: bc => NULL() !< A pointer to the array of boundary condition fields
    logical    :: set = .false.       !< If true, this type has been initialized
    integer    :: isd, isc, iec, ied  !< The i-direction data and computational domain index ranges for this type
    integer    :: jsd, jsc, jec, jed  !< The j-direction data and computational domain index ranges for this type
  end type coupler_2d_bc_type_r8
  ...
  type, public    :: coupler_1d_values_type_r8
    character(len=48)           :: name = ' '  !< The diagnostic name for this array
    real(DOUBLE_KIND), pointer, dimension(:) :: values => NULL() !< The pointer to the array of values
    logical                     :: mean = .true. !< mean
    logical                     :: override = .false. !< override
    integer                     :: id_diag = 0 !< The diagnostic id for this array
    character(len=128)          :: long_name = ' ' !< The diagnostic long_name for this array
    ...
    logical                     :: may_init = .true. !< If true, there is an internal method
                                               !! that can be used to initialize this field
                                               !! if it can not be read from a restart file
  end type coupler_1d_values_type_r8
  ...
  type, public    :: coupler_1d_field_type_r8
    character(len=48)              :: name = ' ' !< name
    integer                        :: num_fields = 0 !< num_fields
    type(coupler_1d_values_type_r8), pointer, dimension(:)   :: field => NULL() !< field
    character(len=128)             :: flux_type = ' ' !< flux_type
    character(len=128)             :: implementation = ' ' !< implementation
    real(DOUBLE_KIND), pointer, dimension(:)    :: param => NULL() !< param
    logical, pointer, dimension(:) :: flag => NULL() !< flag
    ...
    logical                        :: pass_through_ice !< pass_through_ice
    real(DOUBLE_KIND)              :: mol_wt = 0.0d0 !< mol_wt
  end type coupler_1d_field_type_r8
  ...
  type, public    :: coupler_1d_bc_type_r8
    integer                                            :: num_bcs = 0  !< The number of boundary condition fields
    type(coupler_1d_field_type_r8), dimension(:), pointer :: bc => NULL() !< A pointer to the array of boundary condition fields
    logical    :: set = .false.       !< If true, this type has been initialized
  end type coupler_1d_bc_type_r8
  ...
  interface  coupler_type_copy
    ...
    module procedure coupler_type_copy_1d_2d_r8, coupler_type_copy_1d_3d_r8
    module procedure coupler_type_copy_2d_2d_r8, coupler_type_copy_2d_3d_r8
    module procedure coupler_type_copy_3d_2d_r8, coupler_type_copy_3d_3d_r8
  ...
  interface  coupler_type_spawn
    ...
    module procedure CT_spawn_1d_2d_r8, CT_spawn_2d_2d_r8, CT_spawn_3d_2d_r8
    module procedure CT_spawn_1d_3d_r8, CT_spawn_2d_3d_r8, CT_spawn_3d_3d_r8
  ...
  interface coupler_type_copy_data
    ...
    module procedure CT_copy_data_2d_r8, CT_copy_data_3d_r8, CT_copy_data_2d_3d_r8
  ...
  interface coupler_type_increment_data
    ...
    module procedure CT_increment_data_2d_2d_r8, CT_increment_data_3d_3d, CT_increment_data_2d_3d_r8
  ...
  interface coupler_type_set_diags
    ...
    module procedure CT_set_diags_2d_r8, CT_set_diags_3d_r8
  ...
  interface coupler_type_register_restarts
    ...
    module procedure CT_register_restarts_to_file_2d_r8, CT_register_restarts_to_file_3d_r8
  ...
  interface coupler_type_restore_state
    ...
    module procedure CT_restore_state_2d_r8, CT_restore_state_3d_r8
  ...
  interface coupler_type_initialized
    ...
    module procedure CT_initialized_1d_r8, CT_initialized_2d_r8, CT_initialized_3d_r8
  ...
  interface coupler_type_destructor
    ...
    module procedure CT_destructor_1d_r8, CT_destructor_2d_r8, CT_destructor_3d_r8
...
contains
  ...
  subroutine coupler_type_copy_1d_2d_r8(var_in, var_out, is, ie, js, je,&
      & diag_name, axes, time, suffix)
    type(coupler_1d_bc_type_r8), intent(in)    :: var_in !< variable to copy information from
    type(coupler_2d_bc_type_r8), intent(inout) :: var_out !< variable to copy information to
    integer, intent(in)                     :: is !< lower bound of first dimension
    integer, intent(in)                     :: ie !< upper bound of first dimension
    integer, intent(in)                     :: js !< lower bound of second dimension
    integer, intent(in)                     :: je !< upper bound of second dimension
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (coupler_type_copy_1d_2d_r8):'
    character(len=400)      :: error_msg
    integer                 :: m, n
    ...
    if (var_in%num_bcs >= 0)&
        & call CT_spawn_1d_2d_r8(var_in, var_out, (/ is, is, ie, ie /), (/ js, js, je, je /), suffix)

    if ((var_out%num_bcs > 0) .and. (diag_name .ne. ' '))&
        & call CT_set_diags_2d_r8(var_out, diag_name, axes, time)
  end subroutine  coupler_type_copy_1d_2d_r8
  ...
  subroutine coupler_type_copy_1d_3d_r8(var_in, var_out, is, ie, js, je, kd,&
      & diag_name, axes, time, suffix)
    type(coupler_1d_bc_type_r8), intent(in)    :: var_in !< variable to copy information from
    type(coupler_3d_bc_type_r8), intent(inout) :: var_out !< variable to copy information to
    integer, intent(in)                     :: is !< lower bound of first dimension
    integer, intent(in)                     :: ie !< upper bound of first dimension
    integer, intent(in)                     :: js !< lower bound of second dimension
    integer, intent(in)                     :: je !< upper bound of second dimension
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (coupler_type_copy_1d_3d_r8):'
    character(len=400)      :: error_msg
    integer                 :: m, n
    ...
    if (var_in%num_bcs >= 0)&
        & call CT_spawn_1d_3d_r8(var_in, var_out,  (/ is, is, ie, ie /), (/ js, js, je, je /), (/1, kd/), suffix)

    if ((var_out%num_bcs > 0) .and. (diag_name .ne. ' '))&
        & call CT_set_diags_3d_r8(var_out, diag_name, axes, time)
  end subroutine  coupler_type_copy_1d_3d_r8
  ...
  subroutine coupler_type_copy_2d_2d_r8(var_in, var_out, is, ie, js, je,&
      & diag_name, axes, time, suffix)
    type(coupler_2d_bc_type_r8), intent(in)    :: var_in !< variable to copy information from
    type(coupler_2d_bc_type_r8), intent(inout) :: var_out !< variable to copy information to
    integer, intent(in)                     :: is !< lower bound of first dimension
    integer, intent(in)                     :: ie !< upper bound of first dimension
    integer, intent(in)                     :: js !< lower bound of second dimension
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (coupler_type_copy_2d_2d_r8):'
    character(len=400)      :: error_msg
    integer                 :: m, n
    ...
    if (var_in%num_bcs >= 0)&
        & call CT_spawn_2d_2d_r8(var_in, var_out, (/ is, is, ie, ie /), (/ js, js, je, je /), suffix)

    if ((var_out%num_bcs > 0) .and. (diag_name .ne. ' '))&
        & call CT_set_diags_2d_r8(var_out, diag_name, axes, time)
  end subroutine  coupler_type_copy_2d_2d_r8
  ...
  subroutine coupler_type_copy_2d_3d_r8(var_in, var_out, is, ie, js, je, kd,&
      & diag_name, axes, time, suffix)
    type(coupler_2d_bc_type_r8), intent(in)    :: var_in !< variable to copy information from
    type(coupler_3d_bc_type_r8), intent(inout) :: var_out !< variable to copy information to
    integer, intent(in)                     :: is !< lower bound of first dimension
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (coupler_type_copy_2d_3d_r8):'
    character(len=400)      :: error_msg
    integer                 :: m, n
    ...
    if (var_in%num_bcs >= 0)&
        & call CT_spawn_2d_3d_r8(var_in, var_out,  (/ is, is, ie, ie /), (/ js, js, je, je /), (/1, kd/), suffix)

    if ((var_out%num_bcs > 0) .and. (diag_name .ne. ' '))&
        & call CT_set_diags_3d_r8(var_out, diag_name, axes, time)
  end subroutine  coupler_type_copy_2d_3d_r8
  ...
  subroutine coupler_type_copy_3d_2d_r8(var_in, var_out, is, ie, js, je,&
      & diag_name, axes, time, suffix)
    type(coupler_3d_bc_type_r8), intent(in)    :: var_in !< variable to copy information from
    type(coupler_2d_bc_type_r8), intent(inout) :: var_out !< variable to copy information to
    integer, intent(in)                     :: is !< lower bound of first dimension
    integer, intent(in)                     :: ie !< upper bound of first dimension
    integer, intent(in)                     :: js !< lower bound of second dimension
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (coupler_type_copy_3d_2d_r8):'
    character(len=400)      :: error_msg
    integer                 :: m, n
    ...
    if (var_in%num_bcs >= 0)&
        & call CT_spawn_3d_2d_r8(var_in, var_out, (/ is, is, ie, ie /), (/ js, js, je, je /), suffix)

    if ((var_out%num_bcs > 0) .and. (diag_name .ne. ' '))&
        & call CT_set_diags_2d_r8(var_out, diag_name, axes, time)
  end subroutine  coupler_type_copy_3d_2d_r8
  ...
  subroutine coupler_type_copy_3d_3d_r8(var_in, var_out, is, ie, js, je, kd,&
      & diag_name, axes, time, suffix)
    type(coupler_3d_bc_type_r8), intent(in)    :: var_in !< variable to copy information from
    type(coupler_3d_bc_type_r8), intent(inout) :: var_out !< variable to copy information to
    integer, intent(in)                     :: is !< lower bound of first dimension
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (coupler_type_copy_3d_3d_r8):'
    character(len=400)      :: error_msg
    integer                 :: m, n
    ...
    if (var_in%num_bcs >= 0)&
        & call CT_spawn_3d_3d_r8(var_in, var_out,  (/ is, is, ie, ie /), (/ js, js, je, je /), (/1, kd/), suffix)

    if ((var_out%num_bcs > 0) .and. (diag_name .ne. ' '))&
        & call CT_set_diags_3d_r8(var_out, diag_name, axes, time)
  end subroutine  coupler_type_copy_3d_3d_r8
  ...
  subroutine CT_spawn_1d_2d_r8(var_in, var, idim, jdim, suffix, as_needed)
    type(coupler_1d_bc_type_r8), intent(in)    :: var_in  !< structure from which to copy information
    type(coupler_2d_bc_type_r8), intent(inout) :: var     !< structure into which to copy information
    integer, dimension(4),    intent(in)    :: idim    !< The data and computational domain extents of
                                                       !! the first dimension in a non-decreasing list
    integer, dimension(4),    intent(in)    :: jdim    !< The data and computational domain extents of
                                                       !! the second dimension in a non-decreasing list
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (CT_spawn_1d_2d_r8):'
    character(len=400)      :: error_msg
    integer                 :: m, n
  ...
  end subroutine  CT_spawn_1d_2d_r8
  ...
  subroutine CT_spawn_1d_3d_r8(var_in, var, idim, jdim, kdim, suffix, as_needed)
    type(coupler_1d_bc_type_r8), intent(in)    :: var_in  !< structure from which to copy information
    type(coupler_3d_bc_type_r8), intent(inout) :: var     !< structure into which to copy information
    integer, dimension(4),    intent(in)    :: idim    !< The data and computational domain extents of
                                                       !! the first dimension in a non-decreasing list
    integer, dimension(4),    intent(in)    :: jdim    !< The data and computational domain extents of
                                                       !! the second dimension in a non-decreasing list
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (CT_spawn_1d_3d_r8):'
    character(len=400)      :: error_msg
    integer                 :: m, n
  ...
  end subroutine  CT_spawn_1d_3d_r8
  ...
  subroutine CT_spawn_2d_2d_r8(var_in, var, idim, jdim, suffix, as_needed)
    type(coupler_2d_bc_type_r8), intent(in)    :: var_in  !< structure from which to copy information
    type(coupler_2d_bc_type_r8), intent(inout) :: var     !< structure into which to copy information
    integer, dimension(4),    intent(in)    :: idim    !< The data and computational domain extents of
                                                       !! the first dimension in a non-decreasing list
    integer, dimension(4),    intent(in)    :: jdim    !< The data and computational domain extents of
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (CT_spawn_2d_2d_r8):'
    character(len=400)      :: error_msg
    integer                 :: m, n
  ...
  end subroutine  CT_spawn_2d_2d_r8
  ...
  subroutine CT_spawn_2d_3d_r8(var_in, var, idim, jdim, kdim, suffix, as_needed)
    type(coupler_2d_bc_type_r8), intent(in)    :: var_in  !< structure from which to copy information
    type(coupler_3d_bc_type_r8), intent(inout) :: var     !< structure into which to copy information
    integer, dimension(4),    intent(in)    :: idim    !< The data and computational domain extents of
                                                       !! the first dimension in a non-decreasing list
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (CT_spawn_2d_3d_r8):'
    character(len=400)      :: error_msg
    integer                 :: m, n
  ...
  end subroutine  CT_spawn_2d_3d_r8
  ...
  subroutine CT_spawn_3d_2d_r8(var_in, var, idim, jdim, suffix, as_needed)
    type(coupler_3d_bc_type_r8), intent(in)    :: var_in  !< structure from which to copy information
    type(coupler_2d_bc_type_r8), intent(inout) :: var     !< structure into which to copy information
    integer, dimension(4),    intent(in)    :: idim    !< The data and computational domain extents of
                                                       !! the first dimension in a non-decreasing list
    integer, dimension(4),    intent(in)    :: jdim    !< The data and computational domain extents of
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (CT_spawn_3d_2d_r8):'
    character(len=400)      :: error_msg
    integer                 :: m, n
  ...
  end subroutine  CT_spawn_3d_2d_r8
...
!!
  ...
  subroutine CT_spawn_3d_3d_r8(var_in, var, idim, jdim, kdim, suffix, as_needed)
    type(coupler_3d_bc_type_r8), intent(in)    :: var_in  !< structure from which to copy information
    type(coupler_3d_bc_type_r8), intent(inout) :: var     !< structure into which to copy information
    integer, dimension(4),    intent(in)    :: idim    !< The data and computational domain extents of
                                                       !! the first dimension in a non-decreasing list
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (CT_spawn_3d_3d_r8):'
    character(len=400)      :: error_msg
    integer                 :: m, n
  ...
  end subroutine  CT_spawn_3d_3d_r8
  ...
  subroutine CT_copy_data_2d_r8(var_in, var, halo_size, bc_index, field_index,&
      & exclude_flux_type, only_flux_type, pass_through_ice)
    type(coupler_2d_bc_type_r8),   intent(in)    :: var_in  !< BC_type structure with the data to copy
    type(coupler_2d_bc_type_r8),   intent(inout) :: var !< The recipient BC_type structure
    integer,          optional, intent(in)    :: halo_size !< The extent of the halo to copy; 0 by default
    integer,          optional, intent(in)    :: bc_index  !< The index of the boundary condition
                                                           !! that is being copied
    integer,          optional, intent(in)    :: field_index !< The index of the field in the
    ...
    if (present(bc_index)) then
      if (bc_index > var_in%num_bcs)&
          & call mpp_error(FATAL, "CT_copy_data_2d_r8: bc_index is present and exceeds var_in%num_bcs.")
      if (present(field_index)) then ; if (field_index > var_in%bc(bc_index)%num_fields)&
          & call mpp_error(FATAL, "CT_copy_data_2d_r8: field_index is present and exceeds num_fields for" //&
          & trim(var_in%bc(bc_index)%name) )
      endif
    elseif (present(field_index)) then
      call mpp_error(FATAL, "CT_copy_data_2d_r8: bc_index must be present if field_index is present.")
    endif
    ...
    if (n2 >= n1) then
      ! A more consciencious implementation would include a more descriptive error messages.
      if ((var_in%iec-var_in%isc) /= (var%iec-var%isc))&
          & call mpp_error(FATAL, "CT_copy_data_2d_r8: There is an i-direction computational domain size mismatch.")
      if ((var_in%jec-var_in%jsc) /= (var%jec-var%jsc))&
          & call mpp_error(FATAL, "CT_copy_data_2d_r8: There is a j-direction computational domain size mismatch.")
      if ((var_in%isc-var_in%isd < halo) .or. (var_in%ied-var_in%iec < halo))&
          & call mpp_error(FATAL, "CT_copy_data_2d_r8: Excessive i-direction halo size for the input structure.")
      if ((var_in%jsc-var_in%jsd < halo) .or. (var_in%jed-var_in%jec < halo))&
          & call mpp_error(FATAL, "CT_copy_data_2d_r8: Excessive j-direction halo size for the input structure.")
      if ((var%isc-var%isd < halo) .or. (var%ied-var%iec < halo))&
          & call mpp_error(FATAL, "CT_copy_data_2d_r8: Excessive i-direction halo size for the output structure.")
      if ((var%jsc-var%jsd < halo) .or. (var%jed-var%jec < halo))&
          & call mpp_error(FATAL, "CT_copy_data_2d_r8: Excessive j-direction halo size for the output structure.")
    ...
    do n = n1, n2
      ...
      enddo
    enddo
  end subroutine CT_copy_data_2d_r8
  ...
  subroutine CT_copy_data_3d_r8(var_in, var, halo_size, bc_index, field_index,&
      & exclude_flux_type, only_flux_type, pass_through_ice)
    type(coupler_3d_bc_type_r8),   intent(in)    :: var_in  !< BC_type structure with the data to copy
    type(coupler_3d_bc_type_r8),   intent(inout) :: var !< The recipient BC_type structure
    integer,          optional, intent(in)    :: halo_size !< The extent of the halo to copy; 0 by default
    integer,          optional, intent(in)    :: bc_index  !< The index of the boundary condition
                                                           !! that is being copied
    integer,          optional, intent(in)    :: field_index !< The index of the field in the
    ...
    if (present(bc_index)) then
      if (bc_index > var_in%num_bcs) &
          call mpp_error(FATAL, "CT_copy_data_3d_r8: bc_index is present and exceeds var_in%num_bcs.")
      if (present(field_index)) then ; if (field_index > var_in%bc(bc_index)%num_fields)&
          & call mpp_error(FATAL, "CT_copy_data_3d_r8: field_index is present and exceeds num_fields for" //&
          & trim(var_in%bc(bc_index)%name) )
      endif
    elseif (present(field_index)) then
      call mpp_error(FATAL, "CT_copy_data_3d_r8: bc_index must be present if field_index is present.")
    endif
    ...
    if (n2 >= n1) then
      ! A more consciencious implementation would include a more descriptive error messages.
      if ((var_in%iec-var_in%isc) /= (var%iec-var%isc))&
          & call mpp_error(FATAL, "CT_copy_data_3d_r8: There is an i-direction computational domain size mismatch.")
      if ((var_in%jec-var_in%jsc) /= (var%jec-var%jsc))&
          & call mpp_error(FATAL, "CT_copy_data_3d_r8: There is a j-direction computational domain size mismatch.")
      if ((var_in%ke-var_in%ks) /= (var%ke-var%ks))&
          & call mpp_error(FATAL, "CT_copy_data_3d_r8: There is a k-direction computational domain size mismatch.")
      if ((var_in%isc-var_in%isd < halo) .or. (var_in%ied-var_in%iec < halo))&
          & call mpp_error(FATAL, "CT_copy_data_3d_r8: Excessive i-direction halo size for the input structure.")
      if ((var_in%jsc-var_in%jsd < halo) .or. (var_in%jed-var_in%jec < halo))&
          & call mpp_error(FATAL, "CT_copy_data_3d_r8: Excessive j-direction halo size for the input structure.")
      if ((var%isc-var%isd < halo) .or. (var%ied-var%iec < halo))&
          & call mpp_error(FATAL, "CT_copy_data_3d_r8: Excessive i-direction halo size for the output structure.")
      if ((var%jsc-var%jsd < halo) .or. (var%jed-var%jec < halo))&
          & call mpp_error(FATAL, "CT_copy_data_3d_r8: Excessive j-direction halo size for the output structure.")
    ...
    do n = n1, n2
      ...
      enddo
    enddo
  end subroutine CT_copy_data_3d_r8
  ...
  subroutine CT_copy_data_2d_3d_r8(var_in, var, halo_size, bc_index, field_index,&
      & exclude_flux_type, only_flux_type, pass_through_ice,&
      & ind3_start, ind3_end)
    type(coupler_2d_bc_type_r8),   intent(in)    :: var_in  !< BC_type structure with the data to copy
    type(coupler_3d_bc_type_r8),   intent(inout) :: var !< The recipient BC_type structure
    ...
    if (present(bc_index)) then
      if (bc_index > var_in%num_bcs)&
          & call mpp_error(FATAL, "CT_copy_data_2d_3d_r8: bc_index is present and exceeds var_in%num_bcs.")
      if (present(field_index)) then ; if (field_index > var_in%bc(bc_index)%num_fields)&
          & call mpp_error(FATAL, "CT_copy_data_2d_3d_r8: field_index is present and exceeds num_fields for" //&
          & trim(var_in%bc(bc_index)%name) )
      endif
    elseif (present(field_index)) then
      call mpp_error(FATAL, "CT_copy_data_2d_3d_r8: bc_index must be present if field_index is present.")
    endif
    ...
    if (n2 >= n1) then
      ! A more consciencious implementation would include a more descriptive error messages.
      if ((var_in%iec-var_in%isc) /= (var%iec-var%isc))&
          & call mpp_error(FATAL, "CT_copy_data_2d_3d_r8: There is an i-direction computational domain size mismatch.")
      if ((var_in%jec-var_in%jsc) /= (var%jec-var%jsc))&
          & call mpp_error(FATAL, "CT_copy_data_2d_3d_r8: There is a j-direction computational domain size mismatch.")
      if ((var_in%isc-var_in%isd < halo) .or. (var_in%ied-var_in%iec < halo))&
          & call mpp_error(FATAL, "CT_copy_data_2d_3d_r8: Excessive i-direction halo size for the input structure.")
      if ((var_in%jsc-var_in%jsd < halo) .or. (var_in%jed-var_in%jec < halo))&
          & call mpp_error(FATAL, "CT_copy_data_2d_3d_r8: Excessive j-direction halo size for the input structure.")
      if ((var%isc-var%isd < halo) .or. (var%ied-var%iec < halo))&
          & call mpp_error(FATAL, "CT_copy_data_2d_3d_r8: Excessive i-direction halo size for the output structure.")
      if ((var%jsc-var%jsd < halo) .or. (var%jed-var%jec < halo))&
          & call mpp_error(FATAL, "CT_copy_data_2d_3d_r8: Excessive j-direction halo size for the output structure.")
    endif
    ...
    do n = n1, n2
      ...
      enddo
    enddo
  end subroutine CT_copy_data_2d_3d_r8
  ...
  subroutine CT_redistribute_data_2d_r8(var_in, domain_in, var_out, domain_out, complete)
    type(coupler_2d_bc_type_r8), intent(in)    :: var_in     !< BC_type structure with the data to copy (intent in)
    type(domain2D),           intent(in)    :: domain_in  !< The FMS domain for the input structure
    type(coupler_2d_bc_type_r8), intent(inout) :: var_out    !< The recipient BC_type structure (data intent out)
    type(domain2D),           intent(in)    :: domain_out !< The FMS domain for the output structure
    logical,        optional, intent(in)    :: complete   !< If true, complete the updates
  ...
  subroutine CT_redistribute_data_3d_r8(var_in, domain_in, var_out, domain_out, complete)
    type(coupler_3d_bc_type_r8), intent(in)    :: var_in     !< BC_type structure with the data to copy (intent in)
    type(domain2D),           intent(in)    :: domain_in  !< The FMS domain for the input structure
    type(coupler_3d_bc_type_r8), intent(inout) :: var_out    !< The recipient BC_type structure (data intent out)
    type(domain2D),           intent(in)    :: domain_out !< The FMS domain for the output structure
    logical,        optional, intent(in)    :: complete   !< If true, complete the updates
  ...
  subroutine CT_rescale_data_2d_r8(var, scale, halo_size, bc_index, field_index,&
      & exclude_flux_type, only_flux_type, pass_through_ice)
    type(coupler_2d_bc_type_r8),   intent(inout) :: var !< The BC_type structure whose fields are being rescaled
  ...
  subroutine CT_rescale_data_3d_r8(var, scale, halo_size, bc_index, field_index,&
      & exclude_flux_type, only_flux_type, pass_through_ice)
    type(coupler_3d_bc_type_r8),   intent(inout) :: var !< The BC_type structure whose fields are being rescaled
    real(DOUBLE_KIND),                       intent(in)    :: scale   !< A scaling factor to multiply fields by
    integer,          optional, intent(in)    :: halo_size !< The extent of the halo to copy; 0 by default or
                                                           !! the full arrays if scale is 0.
    integer,          optional, intent(in)    :: bc_index  !< The index of the boundary condition
  ...
  subroutine CT_increment_data_2d_2d_r8(var_in, var, halo_size, bc_index, field_index,&
      & scale_factor, scale_prev, exclude_flux_type, only_flux_type, pass_through_ice)
    type(coupler_2d_bc_type_r8),   intent(in)    :: var_in  !< BC_type structure with the data to add to the other type
    type(coupler_2d_bc_type_r8),   intent(inout) :: var !< The BC_type structure whose fields are being incremented
    integer,          optional, intent(in)    :: halo_size !< The extent of the halo to increment; 0 by default
    integer,          optional, intent(in)    :: bc_index  !< The index of the boundary condition
                                                         !! that is being copied
    integer,          optional, intent(in)    :: field_index !< The index of the field in the
  ...
  subroutine CT_increment_data_3d_3d_r8(var_in, var, halo_size, bc_index, field_index,&
      & scale_factor, scale_prev, exclude_flux_type, only_flux_type, pass_through_ice)
    type(coupler_3d_bc_type_r8),   intent(in)    :: var_in  !< BC_type structure with the data to add to the other type
    type(coupler_3d_bc_type_r8),   intent(inout) :: var !< The BC_type structure whose fields are being incremented
    integer,          optional, intent(in)    :: halo_size !< The extent of the halo to copy; 0 by default
    integer,          optional, intent(in)    :: bc_index !< The index of the boundary condition
                                                         !! that is being copied
    integer,          optional, intent(in)    :: field_index !< The index of the field in the
  ...
  subroutine CT_extract_data_2d_r8(var_in, bc_index, field_index, array_out,&
      & scale_factor, halo_size, idim, jdim)
    type(coupler_2d_bc_type_r8),   intent(in)    :: var_in    !< BC_type structure with the data to extract
    integer,                    intent(in)    :: bc_index  !< The index of the boundary condition
                                                           !! that is being copied
    integer,                    intent(in)    :: field_index !< The index of the field in the
                                                           !! boundary condition that is being copied
  ...
  subroutine CT_extract_data_3d_2d_r8(var_in, bc_index, field_index, k_in, array_out,&
      & scale_factor, halo_size, idim, jdim)
    type(coupler_3d_bc_type_r8),   intent(in)    :: var_in    !< BC_type structure with the data to extract
    integer,                    intent(in)    :: bc_index  !< The index of the boundary condition
                                                           !! that is being copied
    integer,                    intent(in)    :: field_index !< The index of the field in the
                                                           !! boundary condition that is being copied
  ...
  subroutine CT_extract_data_3d_r8(var_in, bc_index, field_index, array_out,&
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (CT_extract_data_3d_r8):'
    character(len=400) :: error_msg
  ...
  subroutine CT_set_data_2d_r8(array_in, bc_index, field_index, var,&
    ...
    integer,                    intent(in)    :: field_index !< The index of the field in the
                                                           !! boundary condition that is being copied
    type(coupler_2d_bc_type_r8),   intent(inout) :: var       !< BC_type structure with the data to set
    real(DOUBLE_KIND),             optional, intent(in)    :: scale_factor !< A scaling factor for the data that is being added
    integer,          optional, intent(in)    :: halo_size !< The extent of the halo to copy; 0 by default
    integer, dimension(4), optional, intent(in) :: idim    !< The data and computational domain extents of
                                                           !! the first dimension of the output array
    ...
    integer, dimension(4), optional, intent(in) :: jdim    !< The data and computational domain extents of
                                                           ...
                                                           !! in a non-decreasing list
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (CT_set_data_2d_r8):'
    character(len=400) :: error_msg
  ...
  subroutine CT_set_data_2d_3d_r8(array_in, bc_index, field_index, k_out, var,&
    ...
    integer,                    intent(in)    :: field_index !< The index of the field in the
                                                           !! boundary condition that is being copied
    integer,                    intent(in)    :: k_out     !< The k-index to set
    type(coupler_3d_bc_type_r8),   intent(inout) :: var       !< BC_type structure with the data to be set
    real(DOUBLE_KIND),             optional, intent(in)    :: scale_factor !< A scaling factor for the data that is being added
    integer,          optional, intent(in)    :: halo_size !< The extent of the halo to copy; 0 by default
    integer, dimension(4), optional, intent(in) :: idim    !< The data and computational domain extents of
                                                           !! the first dimension of the output array
    ...
    character(len=*), parameter :: error_header =&
        & '==>Error from coupler_types_mod (CT_set_data_2d_3d_r8):'
    character(len=400)      :: error_msg
  ...
  subroutine CT_set_diags_2d_r8(var, diag_name, axes, time)
    type(coupler_2d_bc_type_r8), intent(inout) :: var  !< BC_type structure for which to register diagnostics
    character(len=*),         intent(in)    :: diag_name !< name for diagnostic file--if blank, then don't register the fields
    integer, dimension(:),    intent(in)    :: axes !< array of axes identifiers for diagnostic variable registration
    type(time_type),          intent(in)    :: time !< model time variable for registering diagnostic field
    ...
    do n = 1, var%num_bcs
      ...
      enddo
    enddo
  end subroutine CT_set_diags_2d_r8
  ...
  subroutine CT_set_diags_3d_r8(var, diag_name, axes, time)
    type(coupler_3d_bc_type_r8), intent(inout) :: var  !< BC_type structure for which to register diagnostics
    character(len=*),         intent(in)    :: diag_name !< name for diagnostic file--if blank, then don't register the fields
    integer, dimension(:),    intent(in)    :: axes !< array of axes identifiers for diagnostic variable registration
    type(time_type),          intent(in)    :: time !< model time variable for registering diagnostic field
    ...
    do n = 1, var%num_bcs
      ...
      enddo
    enddo
  end subroutine CT_set_diags_3d_r8
  ...
  subroutine CT_send_data_2d_r8(var, Time)
    type(coupler_2d_bc_type_r8), intent(in) :: var  !< BC_type structure with the diagnostics to write
    type(time_type),          intent(in) :: time !< The current model time
  ...
  subroutine CT_send_data_3d_r8(var, Time)
    type(coupler_3d_bc_type_r8), intent(in) :: var  !< BC_type structure with the diagnostics to write
    type(time_type),          intent(in) :: time !< The current model time
  ...
  subroutine CT_register_restarts_2d_r8(var, bc_rest_files, num_rest_files, mpp_domain, ocean_restart)
    type(coupler_2d_bc_type_r8), intent(inout) :: var  !< BC_type structure to be registered for restarts
    type(restart_file_type_r8),  dimension(:), pointer :: bc_rest_files !< Structures describing the restart files
    integer,                  intent(out) :: num_rest_files !< The number of restart files to use
    type(domain2D),           intent(in)  :: mpp_domain     !< The FMS domain to use for this registration call
    logical,        optional, intent(in)  :: ocean_restart  !< If true, use the ocean restart file name.
  ...
  subroutine CT_register_restarts_to_file_2d_r8(var, file_name, rest_file, mpp_domain, varname_prefix)
    type(coupler_2d_bc_type_r8), intent(inout) :: var  !< BC_type structure to be registered for restarts
    character(len=*),         intent(in)    :: file_name !< The name of the restart file
    type(restart_file_type_r8),  pointer       :: rest_file !< A (possibly associated) structure describing the restart file
    type(domain2D),           intent(in)    :: mpp_domain !< The FMS domain to use for this registration call
    character(len=*), optional, intent(in)  :: varname_prefix !< A prefix for the variable name
                                                         !! in the restart file, intended to allow
                                                         !! multiple BC_type variables to use the
    ...
    do n = 1, var%num_bcs
      ...
      enddo
    enddo
  end subroutine CT_register_restarts_to_file_2d_r8
  ...
  subroutine CT_register_restarts_3d_r8(var, bc_rest_files, num_rest_files, mpp_domain, ocean_restart)
    type(coupler_3d_bc_type_r8), intent(inout) :: var  !< BC_type structure to be registered for restarts
    type(restart_file_type_r8),  dimension(:), pointer :: bc_rest_files !< Structures describing the restart files
    integer,                  intent(out)   :: num_rest_files !< The number of restart files to use
    type(domain2D),           intent(in)    :: mpp_domain     !< The FMS domain to use for this registration call
    logical,        optional, intent(in)    :: ocean_restart  !< If true, use the ocean restart file name.
  ...
  subroutine CT_register_restarts_to_file_3d_r8(var, file_name, rest_file, mpp_domain, varname_prefix)
    type(coupler_3d_bc_type_r8), intent(inout) :: var  !< BC_type structure to be registered for restarts
    character(len=*),         intent(in)  :: file_name !< The name of the restart file
    type(restart_file_type_r8),  pointer     :: rest_file !< A (possibly associated) structure describing the restart file
    type(domain2D),           intent(in)  :: mpp_domain !< The FMS domain to use for this registration call
    ...
    do n = 1, var%num_bcs
      ...
      enddo
    enddo
  end subroutine CT_register_restarts_to_file_3d_r8
  ...
  subroutine CT_restore_state_2d_r8(var, directory, all_or_nothing, all_required, test_by_field)
    type(coupler_2d_bc_type_r8), intent(inout) :: var  !< BC_type structure to restore from restart files
    character(len=*), optional, intent(in)  :: directory !< A directory where the restart files should
                                                    !! be found.  The default for FMS is 'INPUT'.
    logical,        optional, intent(in)    :: all_or_nothing !< If true and there are non-mandatory
                                                    !! restart fields, it is still an error if some
  ...
  end subroutine CT_restore_state_2d_r8
  ...
  subroutine CT_restore_state_3d_r8(var, directory, all_or_nothing, all_required, test_by_field)
    type(coupler_3d_bc_type_r8), intent(inout) :: var  !< BC_type structure to restore from restart files
    character(len=*), optional, intent(in)  :: directory !< A directory where the restart files should
                                                    !! be found.  The default for FMS is 'INPUT'.
    logical,        optional, intent(in)    :: all_or_nothing !< If true and there are non-mandatory
                                                    !! restart fields, it is still an error if some
  ...
  end subroutine CT_restore_state_3d_r8
  ...
  subroutine CT_data_override_2d_r8(gridname, var, Time)
    character(len=3),         intent(in)    :: gridname !< 3-character long model grid ID
    type(coupler_2d_bc_type_r8), intent(inout) :: var  !< BC_type structure to override
    type(time_type),          intent(in)    :: time !< The current model time
  ...
  subroutine CT_data_override_3d_r8(gridname, var, Time)
    character(len=3),         intent(in)    :: gridname !< 3-character long model grid ID
    type(coupler_3d_bc_type_r8), intent(inout) :: var  !< BC_type structure to override
    type(time_type),          intent(in)    :: time !< The current model time
  ...
  subroutine CT_write_chksums_2d_r8(var, outunit, name_lead)
    type(coupler_2d_bc_type_r8),   intent(in) :: var  !< BC_type structure for which to register diagnostics
    integer,                    intent(in) :: outunit !< The index of a open output file
    character(len=*), optional, intent(in) :: name_lead !< An optional prefix for the variable names
  ...
  subroutine CT_write_chksums_3d_r8(var, outunit, name_lead)
    type(coupler_3d_bc_type_r8),   intent(in) :: var  !< BC_type structure for which to register diagnostics
    integer,                    intent(in) :: outunit !< The index of a open output file
    character(len=*), optional, intent(in) :: name_lead !< An optional prefix for the variable names
  ...
  logical function CT_initialized_1d_r8(var)
    type(coupler_1d_bc_type_r8), intent(in) :: var  !< BC_type structure to be deconstructed

    CT_initialized_1d_r8 = var%set
  end function CT_initialized_1d_r8
  ...
  logical function CT_initialized_2d_r8(var)
    type(coupler_2d_bc_type_r8), intent(in) :: var  !< BC_type structure to be deconstructed

    CT_initialized_2d_r8 = var%set
  end function CT_initialized_2d_r8
  ...
  logical function CT_initialized_3d_r8(var)
    type(coupler_3d_bc_type_r8), intent(in) :: var  !< BC_type structure to be deconstructed

    CT_initialized_3d_r8 = var%set
  end function CT_initialized_3d_r8
  ...
  subroutine CT_destructor_1d_r8(var)
    type(coupler_1d_bc_type_r8), intent(inout) :: var  !< BC_type structure to be deconstructed
  ...
  subroutine CT_destructor_2d_r8(var)
    type(coupler_2d_bc_type_r8), intent(inout) :: var  !< BC_type structure to be deconstructed
  ...
  subroutine CT_destructor_3d_r8(var)
    type(coupler_3d_bc_type_r8), intent(inout) :: var  !< BC_type structure to be deconstructed
