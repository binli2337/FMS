...
#include <fms_platform.h>
  ...
  use coupler_types_mod, only: coupler_1d_bc_type_r8
  ...
  interface  aof_set_coupler_flux
  module procedure  aof_set_coupler_flux_r
  module procedure  aof_set_coupler_flux_r8
  end interface aof_set_coupler_flux
...
contains
  ...
  function aof_set_coupler_flux_r(name, flux_type, implementation, atm_tr_index, param, flag,&
    ...
    return
  end function aof_set_coupler_flux_r

  function aof_set_coupler_flux_r8(name, flux_type, implementation, atm_tr_index, param, flag,&
      & mol_wt, ice_restart_file, ocean_restart_file, units, caller, verbosity) &
      & result (coupler_index)
    character(len=*), intent(in)                :: name !< name
    character(len=*), intent(in)                :: flux_type !< flux_type
    character(len=*), intent(in)                :: implementation !< implementation
    integer, intent(in), optional               :: atm_tr_index !< atm_tr_index
    real(DOUBLE_KIND), intent(in), dimension(:), optional    :: param !< param
    logical, intent(in), dimension(:), optional :: flag !< flag
    real(DOUBLE_KIND), intent(in), optional     :: mol_wt !< mol_wt
    character(len=*), intent(in), optional      :: ice_restart_file !< ice_restart_file
    character(len=*), intent(in), optional      :: ocean_restart_file !< ocean_restart_file
    character(len=*), intent(in), optional      :: units !< units
    character(len=*), intent(in), optional      :: caller !< caller
    ...
    character(len=*), parameter  :: sub_name = 'aof_set_coupler_flux_r8'
    ...
    return
  end function aof_set_coupler_flux_r8
